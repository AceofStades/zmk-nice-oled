...
static void set_battery_status(struct zmk_widget_screen *widget, struct battery_info state) {
    if (state.source > CONFIG_ZMK_SPLIT_BLE_CENTRAL_PERIPHERALS) return;
...
static struct battery_info peripheral_battery_status_get_state(const zmk_event_t *eh) {
...
static struct battery_info central_battery_status_get_state(const zmk_event_t *eh) {
...
static struct battery_info battery_status_get_state(const zmk_event_t *eh) {
...
static void battery_status_update_cb(struct battery_info state) {
    struct zmk_widget_screen *widget;
    SYS_SLIST_FOR_EACH_CONTAINER(&widgets, widget, node) { set_battery_status(widget, state); }
}

ZMK_DISPLAY_WIDGET_LISTENER(widget_battery_status, struct battery_info,
...
int zmk_widget_screen_init(struct zmk_widget_screen *widget, lv_obj_t *parent) {
    ...
    lv_obj_align(middle, LV_ALIGN_TOP_RIGHT, BUFFER_OFFSET_MIDDLE - 16, 0);
    lv_canvas_set_buffer(middle, widget->cbuf2, BUFFER_SIZE, BUFFER_SIZE, LV_IMG_CF_TRUE_COLOR);

    lv_obj_t *bottom = lv_canvas_create(widget->obj);
    lv_obj_align(bottom, LV_ALIGN_TOP_RIGHT, BUFFER_OFFSET_BOTTOM - 16, 0);
